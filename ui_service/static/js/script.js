document.addEventListener('DOMContentLoaded', () => {
    ...
    async function handleSendMessage() {
        ...
        try {
            ...
            handleResponseWithContext(data.results);
    ...
    async function visualizeInventoryPipeline() {
        try {
            ...
            if (data.status === 'success') {
                ...
                conversationContext.updateInventoryContext(data);
                
                // Create visualizations
                const chartData = {
                    labels: ['A-Class', 'B-Class', 'C-Class'],
                    datasets: [{
                        label: 'Items by Classification',
                        data: [
                            data.summary.items_by_class.A,
                            data.summary.items_by_class.B,
                            data.summary.items_by_class.C
                        ],
                        backgroundColor: [
                            'rgba(255, 99, 132, 0.5)',
                            'rgba(54, 162, 235, 0.5)',
                ...
                const stockStatusData = {
                    ...
                    datasets: [{
                        label: 'Items by Stock Status',
                        data: [
                            data.summary.items_by_status.normal,
                            data.summary.items_by_status.low_stock,
                            data.summary.items_by_status.out_of_stock
                        ],
                        backgroundColor: [
                            'rgba(75, 192, 192, 0.5)',
                            'rgba(255, 159, 64, 0.5)',
                ...
                summaryTable.className = 'inventory-summary';
                summaryTable.innerHTML = `
                    ...
                    <table>
                        ...
                        <tr>
                            <td>A-Class Items</td>
                            <td>${data.summary.items_by_class.A}</td>
                        </tr>
                        <tr>
                            <td>B-Class Items</td>
                            <td>${data.summary.items_by_class.B}</td>
                        </tr>
                        <tr>
                            <td>C-Class Items</td>
                            <td>${data.summary.items_by_class.C}</td>
                        </tr>
                        <tr>
                            <td>Refrigerated Items</td>
                            <td>${data.summary.special_handling.refrigerated}</td>
                        </tr>
                        <tr>
                            <td>Special Handling Required</td>
                            <td>${data.summary.special_handling.special_handling}</td>
                        </tr>
                    </table>
                `;
                ...
                detailedTable.className = 'inventory-details';
                detailedTable.innerHTML = `
                    ...
                    <table>
                        ...
                        ${data.items.map(item => `
                            <tr class="status-${item.StockStatus.toLowerCase().replace(' ', '-')}">
                ...
                if (container) {
                    ...
                    // Add classification chart
                    const classChartDiv = document.createElement('div');
                    classChartDiv.className = 'chart-container';
                    const classCanvas = document.createElement('canvas');
                    classCanvas.id = 'classification-chart';
                    classChartDiv.appendChild(classCanvas);
                    container.appendChild(classChartDiv);
                    
                    // Add stock status chart
                    const statusChartDiv = document.createElement('div');
                    statusChartDiv.className = 'chart-container';
                    const statusCanvas = document.createElement('canvas');
                    statusCanvas.id = 'status-chart';
                    statusChartDiv.appendChild(statusCanvas);
            ...
            console.error('Error in inventory pipeline:', error);
            appendMessage('assistant', 'Error visualizing inventory data. Please try again.');
        }
    }
}); 